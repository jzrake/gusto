...
int gusto_user_set_from_arg(struct gusto_user *user, char *arg)
{
	...
	if (num != 3) {
	    printf("[gusto] error: badly formatted option 'N'\n");
	    error += 1;
	}
	...
	if (num != 3) {
	    printf("[gusto] error: badly formatted option 'abc'\n");
	    error += 1;
	}
	...
	printf("[gusto] error: no such option '%s'\n", arg);
	error = 1;
    }
    return error;
}
...
{
    memset(user, 0, sizeof(struct gusto_user));
...
{
    memset(status, 0, sizeof(struct gusto_status));
    status->iteration = 0;
...
#define ADD_MEM(nm, tp) H5Tinsert(h5t, #nm, HOFFSET(struct gusto_status, nm), tp)

    hid_t h5f = 0;
    if (mode == 'a' || mode == 'r') {
	if (!H5Fis_hdf5(chkpt_name)) {
	    printf("[gusto] error: '%s' does not exist\n", chkpt_name);
	    return 1;
	}
	h5f = H5Fopen(chkpt_name, H5F_ACC_RDWR, H5P_DEFAULT);
    } else if (mode == 'w') {
	h5f =
	    H5Fcreate(chkpt_name, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    }
    hid_t h5s = H5Screate(H5S_SCALAR);
    hid_t h5t = H5Tcreate(H5T_COMPOUND, sizeof(struct gusto_status));
...
#define ADD_MEM(nm, tp) H5Tinsert(h5t, #nm, HOFFSET(struct gusto_user, nm), tp)
    ...
    hid_t h5s = H5Screate(H5S_SCALAR);
    hid_t h5t = H5Tcreate(H5T_COMPOUND, sizeof(struct gusto_user));
		    ...
		    const char *type_name)
{
	...
	for (int m = 0; m < src_nmembers; ++m) {
		...
		if (H5Tequal(src_member_type, dst_member_type) <= 0) {
		    ...
		    printf
			("[gusto] error: incompatible type for member %s:%s\n",
	...
	if (found == 0) {
	    printf("[gusto] warning: %s:%s not found in source data\n",
		   type_name, member_name);
	}
		      ...
		      char mode, char *name, void *data)
{
	...
	if (H5Tequal(src_type, h5t) <= 0) {
	    ...
	    printf("[gusto] warning: checkpoint '%s' out of date\n", name);

	    if (read_old_h5type(h5d, h5t, data, "status")) {
		printf
		    ("[gusto] error: checkpoint '%s' could not be read\n",
		     name);
	    } else {
		error = 0;	/* success */
	    }
